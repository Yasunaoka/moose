...
#include "ComputeLinearElasticPFFractureStressAndNoDegradationEnergy.h"
#include "MathUtils.h"

registerMooseObject("TensorMechanicsApp", ComputeLinearElasticPFFractureStressAndNoDegradationEnergy);

InputParameters
ComputeLinearElasticPFFractureStressAndNoDegradationEnergy::validParams()
{
...
ComputeLinearElasticPFFractureStressAndNoDegradationEnergy::ComputeLinearElasticPFFractureStressAndNoDegradationEnergy(
...
void
ComputeLinearElasticPFFractureStressAndNoDegradationEnergy::initialSetup()
{
...
void
ComputeLinearElasticPFFractureStressAndNoDegradationEnergy::computeStrainSpectral(Real & F_pos, Real & F_neg)
{
...
void
ComputeLinearElasticPFFractureStressAndNoDegradationEnergy::computeStressSpectral(Real & F_pos, Real & F_neg)
{
...
void
ComputeLinearElasticPFFractureStressAndNoDegradationEnergy::computeStrainVolDev(Real & F_pos, Real & F_neg)
{
  // Isotropic elasticity is assumed and should be enforced
  const Real lambda = _elasticity_tensor[_qp](0, 0, 1, 1);
  const Real mu = _elasticity_tensor[_qp](0, 1, 0, 1);
...
void
ComputeLinearElasticPFFractureStressAndNoDegradationEnergy::computeQpStress()
{
  Real F_pos, F_neg;
  RankTwoTensor I2(RankTwoTensor::initIdentity);
  ...
  // Elastic free energy density, only calculate the degradation part, used in multapp transfer which will lose the dpart of this material
  _E[_qp] =
      hist_variable;
}
